...
y = sunspot(:,2)';
y_data = y;
% محاسبه مقدار کمینه و بیشینه مجموعه
min_number = min(y_data);
max_number = max(y_data);
% نرمال کردن مجموعه
y = mapminmax(y_data, 0, 1);
...
% from normalize
y2_active_h = mapminmax(y2_active_h, min_number, max_number);
d1_all = mapminmax(d1_all, min_number, max_number);
%% plot data
...
function [all_input,valid_input,test_input,all] = train_test_split(y,n,n_v_start,n_v_stop,n_t_start,n_t_stop)
    ...
    for k = (3:n)
        input(1,1) = to_normalize_input(0);
...
function [out,val_out,test_out,all] = output_split(y,train_num,n_v_start,n_v_stop,n_t_start,n_t_stop)
    len = 305;
    out = to_normalize(y(2:train_num));
